#include <iostream>
#include <cstdint> // 用于intptr_t和uintptr_t类型
using namespace std;

int main() // 数组名用途：可以统计整个数组占用内存大小
{
	...
	cout << "数组首地址为：" << (intptr_t)arr << endl; // 将十六进制数强转为十进制数
	cout << "数组首地址为：" << (long)arr << endl;
	// 数组中第一个元素地址 与 数组的首地址 是 重合的！&：取地址符
	cout << "数组中第一个元素地址为：" << &arr[0] << endl;			 //&：查看一个元素的首地址 十六进制表示地址
	cout << "数组中第一个元素地址为：" << (intptr_t)&arr[0] << endl; //&：查看一个元素的首地址 转为十进制的地址表示
	cout << "数组中第二个元素地址为：" << &arr[1] << endl;
	cout << "数组中第二个元素地址为：" << (intptr_t)&arr[1] << endl;
