...
EventLoop::EventLoop()
    : looping_(false), quit_(false), callingPendingFunctors_(false), threadId_(CurrentThread::tid()), // 保存线程ID
      poller_(Poller::newDefaultPoller(this)),                                                        // 保存Poller对象
      wakeupFd_(createEventfd()),                                                                     // 保存wakeupFd
      wakeupChannel_(new Channel(this, wakeupFd_))                                                    // 保存wakeupChannel
{
    LOG_DEBUG << "EventLoop created" << this << "in thread" << threadId_;
    if (t_loopInThisThread)
    {
        LOG_FATAL << "Another EventLoop" << t_loopInThisThread << "exists in this thread " << threadId_; // 打印错误信息
    }
    else
    {
        t_loopInThisThread = this; // 保存当前EventLoop对象
    }
