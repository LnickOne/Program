...
/**
 * 从文件描述符fd读取数据到Buffer缓冲区
 * 注意：Poller工作在LT（水平触发）模式，需要一次性读完数据，否则会重复触发
 * 
 * Buffer缓冲区大小有限，但TCP数据大小未知，需要高效处理
 * 
 * 解决方案：使用readv分散读（scatter read）
 * - 优先读取到Buffer内部的可写空间
 * - 如果Buffer空间不足，使用栈上临时缓冲区(64KB)暂存数据
 * - 然后将临时缓冲区的数据追加到Buffer中
 * 
 * 优点：
 * 1. 减少系统调用次数（一次readv代替多次read）
 * 2. 避免数据丢失（临时缓冲区兜底）
 * 3. 提高接收效率
 **/
